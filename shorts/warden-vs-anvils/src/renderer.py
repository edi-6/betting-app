...
SCULK_LAYER = 8          # block texture layer of sculk (see textures.py)
...
void main(){
    ...
    float mat = %d.0;
    if (lay == 7) mat = %d.0;
    if (lay == 0 || lay == 2) {                 // (sculk is layer 8, untouched)
...
#version 430
uniform mat4 u_vp;
uniform float u_hurt;       // Minecraft's red damage overlay on the standing zombie (first u_hurt_n instances)
uniform int u_hurt_n;
...
void main(){
    ...
    if (gl_InstanceID < u_hurt_n) v_col = v_col * vec3(1.0, 1.0 - 0.6 * u_hurt, 1.0 - 0.6 * u_hurt) + vec3(0.22 * u_hurt, 0.0, 0.0);
    // inner alpha: 255 bone, 128 glowing soul speck; the z colour's alpha flags glowing skin
    float innerMat = i_inner.a > 0.75 ? %d.0 : (i_inner.a > 0.25 ? %d.0 : %d.0);
    v_mat = isSkin ? (i_cz.a > 0.5 ? %d.0 : %d.0) : innerMat;
    gl_Position = u_vp * vec4(p, 1.0);
}
""" % (MAT_BONE, MAT_GLOW, MAT_FLESH, MAT_GLOW, MAT_SKIN)
...
void main(){
    ...
    o_albedo = vec4(sqrt(clamp(pow(c, vec3(2.2)), 0.0, 1.0)), 0.22);    // worn iron: a little sheen
...
void main(){
    ...
    // point lights (none in this video)
    vec3 pl = vec3(0.0);
...
class Renderer:
    ...
    def set_textures(self, block_tex_list, prop_layers, sky, tint_noise, puffs):
        """block_tex_list: 16x16 RGB or RGBA block textures (alpha = glow mask); prop_layers: 16x16 RGBA."""
        ctx = self.ctx
        blocks = []
        ...
        pl = np.stack([np.ascontiguousarray(a, np.uint8) for a in prop_layers])       # (V,16,16,4)
        self.t_props = ctx.texture_array((pl.shape[2], pl.shape[1], pl.shape[0]), 4, pl.tobytes())
        self.t_props.build_mipmaps()
        self.t_props.filter = (moderngl.LINEAR_MIPMAP_LINEAR, moderngl.NEAREST)
    ...
    def set_prop_mesh(self, mesh):
        """Triangles, vertex = pos3 normal3 uv2 layer1."""
        self.prop_vbo = self.ctx.buffer(np.ascontiguousarray(mesh, np.float32).tobytes())
        self.prop_n = len(mesh)

    def _prop_vaos(self, inst_buf):
        ctx = self.ctx
        g = ctx.vertex_array(self.progs['prop'], [
            (self.prop_vbo, '3f 3f 2f 1f', 'in_pos', 'in_nrm', 'in_uv', 'in_layer'),
            (inst_buf, '3f 4f 1f 1f 1f/i', 'i_pos', 'i_quat', 'i_scale', 'i_var', 'i_fade'),
        ])
        s = ctx.vertex_array(self.progs['sh_prop'], [
            (self.prop_vbo, '3f 24x', 'in_pos'),
            (inst_buf, '3f 4f 1f 8x/i', 'i_pos', 'i_quat', 'i_scale'),
        ])
        return g, s

    def render(self, cam, voxels=None, props=None, fx=None, near_center=(0.0, 0.0, 14.0), hurt=(0.0, 0),
               glow=None, glow_dim=1.0):
        ...
        if props is not None and len(props):
            n_prop = len(props)
            pbuf = ctx.buffer(np.ascontiguousarray(props, np.float32).tobytes())
            prop_g, prop_s = self._prop_vaos(pbuf)
        ...
        if prop_s is not None:
            self.progs['sh_prop']['u_lvp'].write(m4(lvp_near))
            prop_s.render(instances=n_prop)
        ctx.polygon_offset = (0.0, 0.0)
        ...
        if vox_g is not None:
            ...
            self.progs['voxel']['u_hurt'] = float(hurt[0])
            self.progs['voxel']['u_hurt_n'] = int(hurt[1])
            vox_g.render(instances=n_vox)
        if prop_g is not None:
            self.progs['prop']['u_vp'].write(m4(vp))
            self.t_props.use(2)
            self.progs['prop']['u_props'] = 2
            prop_g.render(instances=n_prop)
        P['u_is_ground'] = 0.0
