"""Render a clean 1080x1920 thumbnail: the Warden under the cloud of 10,000 anvils a moment before it starts to
fall, with a bold title.
...
VIDEO_T = 0.3                      # the whole cloud still hanging over him
CAM = {'eye': (13.0, -21.0, 16.0), 'target': (0.0, 0.0, 42.0), 'fov': 72.0}
...
def main(out, cam=CAM, video_t=VIDEO_T, width=1080, ss=2.0, title=True):
    ...
    img = M.render_frame(r, cam, sim, False, glow=6.5).astype(np.float32)
    if title:
        # darken the top a touch so the title pops
        top = int(img.shape[0] * 0.29)
        img[:top] *= np.linspace(0.62, 1.0, top)[:, None, None]
        spr = title_sprite(['WARDEN vs', '10,000 ANVILS'], 112 * width // 1080)
