...
of the falling anvils (closest falling anvil to the lens, measured by running the real simulation with the real
slow-motion schedule; anvil-cam shots are exempt since riding with an anvil is their point)."""
import numpy as np
...
SWARM_MIN = 3.0          # closest a falling anvil may pass a fixed camera
...
def check(verbose=True):
    ...
    for rd in rounds:
        ...
        for f in range(n):
            ...
            st = stats.setdefault(si, [1e9, False, 1e9, 1e9])
            st[0] = min(st[0], c)
            st[1] |= inside
            st[2] = min(st[2], swarm_dist(sim, eye))
            # the anvil pile must not swallow the lens either
            cx, cy = S.cell_of(eye[0], eye[1])
            st[3] = min(st[3], eye[2] - float(sim.support(cx, cy)) - 1.0)
        for si, (c, inside, sw, pile) in sorted(stats.items()):
            ok = c > 0 and not inside and sw > SWARM_MIN and pile > 0
            bad += not ok
            if verbose:
                print(f"{'OK ' if ok else 'BAD'} {rd['key']} shot{si}: clearance {c:6.2f}  canopy {inside}  "
                      f"closest anvil {sw:6.2f}  above pile {pile:6.2f}")
    for hi, hs in enumerate(T.hook_shots()):
        ...
        bad += not ok
        if verbose:
            print(f"{'OK ' if ok else 'BAD'} hook{hi}: clearance {worst:6.2f}  canopy {inside}  closest anvil {sw:6.2f}")
    return bad
