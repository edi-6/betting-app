...
slow-motion schedule, shot list, anvil-cam, heartbeat glow and damage overlay. For look development.
...
def shoot(sim, eye, tgt, fov, up, label, hurt=0.0, follow=None, glow=5.0, dim=1.0):
    cam = cam_override or {'eye': eye, 'target': tgt, 'fov': fov, 'up': tuple(up)}
    im = Image.fromarray(M.render_frame(r, cam, sim, False, hurt=hurt, follow=follow, glow=glow, glow_dim=dim))
    d = ImageDraw.Draw(im)
    d.rectangle((0, 0, 60, 14), fill=(0, 0, 0))
    d.text((3, 1), label, fill=(255, 255, 0))
    tiles.append(np.array(im))
    st = np.bincount(sim.state, minlength=5)
    print(f'{label}: sim t {sim.t:.2f} destroyed {sim.destroyed} anvils fall {st[S.FALL]} rest {st[S.REST]} '
          f'blast {st[S.BLAST]} debris {len(sim.dp)} glow {glow:.1f}', flush=True)
...
if key == 'hook':
    ...
    for hs in T.hook_shots():
        ...
        for _ in range(T.sec(hs['sim_start'])):
            sim.step_frame(1.0)
        heart = M.Heart(phase=0.6)
        nf = T.sec(hs['dur'])
        scales = T.frame_scales(hs.get('scale'), nf)
        follow = M.AnvilCam(sim, hs['anvilcam']) if 'anvilcam' in hs else None
        for f in range(nf):
            sim.step_frame(scales[f])
            if follow is not None:
                eye, tgt, fov, up, _ = follow(f / T.FPS)
            else:
                eye, tgt, fov = T.cam_path(hs['keys'], f / T.FPS)
                up = M.UP
            _, gm = heart.step(scales[f] / T.FPS, sim.t, 0.8, False)
            glow = 5.0 * gm + M.boom_charge(rd, sim.t) * 2.0
            if g in want:
                shoot(sim, eye, tgt, fov, up, f'{g / T.FPS:.2f}', follow=follow, glow=glow)
            g += 1
else:
    ...
    for f in range(max(want) + 1):
        ...
        if 'anvilcam' in shot:
            if si not in follows:
                follows[si] = M.AnvilCam(sim, shot['anvilcam'])
            follow = follows[si]
            eye, tgt, fov, up, _ = follow(t - shot['start'])
        else:
            eye, tgt, fov = T.cam_path(shot['keys'], t)
        if f > 0 and hp[f] < hp[f - 1] and f - flash_start >= M.HURT_GAP:
            flash_start = f
        danger = float(np.clip(M.swarm_info(sim, np.array([0.0, 0.0, 20.0]))[0] / 40.0 + (20 - hp[f]) / 25.0, 0, 1))
        _, gm = heart.step(a['scales'][f] / T.FPS, sim.t, danger, hp[f] <= 0)
        glow = 5.0 * gm + M.boom_charge(rd, sim.t) * 2.0
        if f in want:
            shoot(sim, eye, tgt, fov, up, f'{t:.2f} hp{hp[f]}', M.hurt_amount(f - flash_start), follow, glow,
                  heart.dim)
