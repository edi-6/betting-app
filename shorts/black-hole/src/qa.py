...
Brightness jumps that belong to the action are expected and are not reported: the white flash out of the cold
open, the black hole popping up and growing, the creeper flashing white and going off, the Warden's sonic boom,
the collapse into darkness and the final explosion.
...
def main(video, cues_path):
    ...
    for i in range(1, len(cues)):
        c, p = cues[i], cues[i - 1]
        ev = c.get('events', {})
        whites = [b.get('white', 0.0) for b in c.get('bodies', []) if b['name'] == 'creeper']
        pwhites = [b.get('white', 0.0) for b in p.get('bodies', []) if b['name'] == 'creeper']
        if (whites and pwhites and abs(whites[0] - pwhites[0]) > 0.05) or c.get('flash', 0.0) > 0.02 \
                or p.get('flash', 0.0) > 0.02 or any(k in ev for k in ('creeper_boom', 'boom', 'explode', 'spawn',
                                                                       'stage', 'collapse', 'tnt_boom')) \
                or c.get('seg') != p.get('seg'):
