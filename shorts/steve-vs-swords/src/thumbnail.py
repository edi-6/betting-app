"""Render a clean 1080x1920 thumbnail: the 10,000-sword wall about to hit, with a bold title.
...
def main(out):
    ...
    t_snap = 1.6                        # the wall is a moment away from hitting him
    for _ in range(T.sec(t_snap)):
        sim.step_frame()
    vox, sw = sim.instances()
    r, _ = scene.make_renderer(width=1080, height=1920, ss=2.0, shadow_res=4096)
    eye, tgt, fov = T.cam_path(rd['shots'][0]['keys'], t_snap)   # same camera as the finale's first shot
    cam = {'eye': eye, 'target': tgt, 'fov': fov}
    ...
    img[:560] *= grad
    title = title_sprite(['STEVE vs', '10,000 SWORDS'], 118)
