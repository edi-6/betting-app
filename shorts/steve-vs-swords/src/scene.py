...
def sky_data(width=8192, height=2048, coverage=0.37):
    ...
    return _cached(f'sky_{width}x{height}_c{coverage:.2f}_s{sun}_v3.npz', build)['sky'].astype(np.float32)
...
def make_renderer(width=1080, height=1920, ss=1.0, shadow_res=4096, sky_res=(8192, 2048)):
    ...
    r.set_textures([bt[n] for n in W.LAYERS], SW.texture_layers(), sky, tint_noise())
    r.set_static(wd['vertices'], wd['indices'], wd['chunk_list'])
    full, lod = SW.build_meshes()
    r.set_sword_mesh(full, lod)
    return r, wd
