...
SUN_DIR = np.array([0.40, -0.55, 0.73])
SUN_DIR = SUN_DIR / np.linalg.norm(SUN_DIR)

ZENITH = np.array([0.10, 0.28, 0.78])
HORIZON = np.array([0.62, 0.78, 0.98])
...
def make_sky(width=8192, height=2048, seed=11, clouds=True):
    ...
    t = np.clip(dz, 0, 1)
    k = np.power(t, 0.42)
    sky = HORIZON[None, None, :] * (1 - k[..., None]) + ZENITH[None, None, :] * k[..., None]
    mu = dx * SUN_DIR[0] + dy * SUN_DIR[1] + dz * SUN_DIR[2]
    glow = 0.22 * np.power(np.clip(mu, 0, 1), 6) + 0.9 * np.power(np.clip(mu, 0, 1), 300)
    sky = sky + glow[..., None] * np.array([1.0, 0.92, 0.78])
    # horizon brightening band (haze)
    haze = np.exp(-np.clip(dz, 0, None) * 9.0)
    sky = sky * (1 - 0.35 * haze[..., None]) + 0.35 * haze[..., None] * np.array([0.80, 0.88, 0.98])
    below = dz < 0
    sky[below] = np.array([0.66, 0.76, 0.86])
    ...
    sun_col = np.array([1.00, 0.97, 0.92])
    shadow_col = np.array([0.56, 0.62, 0.74])
    ...
    ccol = ccol * far_tint + (1 - far_tint) * np.array([0.80, 0.86, 0.95])
