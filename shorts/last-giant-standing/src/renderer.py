...
#version 430
uniform mat4 u_vp;
uniform int u_ng;            // standing giants: instance ranges [x, y), fx = (hurt, white flash, swell, -)
...
""" % (MAX_GIANTS, MAX_GIANTS, MAX_GIANTS) + QROT + """
void main(){
    vec3 p = qrot(i_quat, in_pos * i_scale) + i_pos;
    float hurt = 0.0, white = 0.0;
    ...
    if (white > 0.0) v_mat = %d.0;
    gl_Position = u_vp * vec4(p, 1.0);
}
""" % (MAT_BONE, MAT_GLOW, MAT_FLESH, MAT_GLOW, MAT_SKIN, MAT_TNT)
...
void main(){
    ...
    bool tnt = u_sheen < 0.0;
    o_albedo = vec4(sqrt(clamp(pow(c, vec3(2.2)), 0.0, 1.0)), tnt ? v_flash : u_sheen);
    o_normal = vec4(normalize(v_nrm) * 0.5 + 0.5, (tnt ? %d.0 : %d.0) / 255.0);
}
""" % (MAT_TNT, MAT_PROP)
...
""" % (MAT_GLOW, MAT_TNT, MAT_LEAF, MAT_GLOW)
...
class Renderer:
    ...
    def _giant_uniforms(self, giants):
        """giants: list of (start, end, hurt, white, swell, centre3) for the standing giants' instance ranges."""
        rng = np.zeros((MAX_GIANTS, 2), np.int32)
        fx = np.zeros((MAX_GIANTS, 4), np.float32)
        fx[:, 2] = 1.0
        cen = np.zeros((MAX_GIANTS, 3), np.float32)
        n = 0
        for (a, b, hurt, white, swell, c) in (giants or [])[:MAX_GIANTS]:
            rng[n] = (a, b)
    ...
    def render(self, cam, voxels=None, props=None, arrows=None, fx=None, near_center=(0.0, 0.0, 14.0),
               giants=None, glow=None, glow_dim=1.0, sculk=(24.0, 0.0)):
