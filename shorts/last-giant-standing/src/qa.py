...
Brightness jumps that belong to the action are expected and are not reported: the creeper flashing white, the
white flash into the line-up, big blasts (the creeper, sonic booms, mass TNT detonations).
...
def main(video, cues_path):
    ...
    for i in range(1, len(cues)):
        c, p = cues[i], cues[i - 1]
        ev = c.get('events', {})
        if abs(c.get('creeper_white', 0.0) - p.get('creeper_white', 0.0)) > 0.05 \
                or 'creeper_boom' in ev or 'boom' in ev or ev.get('explode', [0])[0] > 20 \
