...
class VFX:
    ...
    def step(self, centers):
        """Advance one video frame and add the explosions that happened during it."""
        dt = 1.0 / FPS
        rng = self.rng
        ...
        if n:
            ...
            if n > MAX_PUFF_EXPL:
                ...
                scale = min(2.4, np.sqrt(n / MAX_PUFF_EXPL))
            ...
            fscale = 1.0 if n <= MAX_FLASH else min(2.0, np.sqrt(n / MAX_FLASH))
            k = len(fsel)
            ...
            self.fint = np.concatenate([self.fint, rng.uniform(0.8, 1.0, k)])
            ...
            for b in range(len(keys)):
                ...
                self.lint = np.concatenate([self.lint, [min(6.0, np.sqrt(counts[b]))]])
