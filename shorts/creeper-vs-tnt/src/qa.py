...
Brightness jumps within 3 frames of an explosion are expected (the blast flash) and are not reported.
...
def main(video, cues_path):
    ...
    for i, c in enumerate(cues):
        if c.get('events', {}).get('explode'):
            boom.update((i, i + 1, i + 2, i + 3))
    prev_key = None
    for i, c in enumerate(cues):
    ...
    for i, f in enumerate(frames(video)):
        ...
        if lum_prev is not None and i not in cut and i not in boom:
