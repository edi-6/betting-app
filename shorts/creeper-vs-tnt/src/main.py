"""Render "Creeper vs 10,000 TNT": hook montage + five rounds, HUD, then sound design and final mux.
...
def encode_youtube(video_in, wav_in, out, workdir, bitrate='12M'):
    ...
    subprocess.run([ffmpeg_exe(), '-y', '-loglevel', 'error', '-i', video_in, '-i', wav_in, '-map', '0:v:0', '-map', '1:a:0']
                   + common + ['-pass', '2', '-passlogfile', log, '-c:a', 'aac', '-b:a', '384k', '-ar', '48000', '-ac', '2',
                               '-movflags', '+faststart', '-shortest', '-metadata', 'title=Creeper vs 10,000 TNT', out],
                   check=True)
...
def shake_curve(rd, an):
    ...
    for f in range(n):
        ev = an['events'][f]
        imp = ev.get('explode', [0])[0]
        carve = ev.get('blast', [0])[0] + ev.get('ground', [0])[0]
        shat = ev.get('shatter', [0])[0]
        kick = np.log1p(imp) * 0.45 + np.log1p(carve) * 0.04 + np.log1p(shat) * 0.2
        acc = acc * 0.86 + kick
        s[f] = acc
...
def render_frame(r, cam, sim, preview):
    if sim.ground.dirty or getattr(r, '_ground_owner', None) is not sim:
    ...
    for rd in rounds:
        ...
        for f in range(n):
            ...
            cue = {'seg': rd['key'], 'shot': si, 'round_frame': f, 'events': a['events'][f], 'cam': eye.tolist(),
                   ...
                   'launch': rd['formation']['launch'].min(), 'n_tnt': len(rd['formation']['pos'])}
            emit(img, cue)
