...
MAT_TERRAIN, MAT_LEAF, MAT_SKIN, MAT_FLESH, MAT_BONE, MAT_GROUND, MAT_ARROW = 1, 2, 3, 4, 5, 7, 8
...
class Renderer:
    def __init__(self, width, height, ss=1.0, shadow_res=4096, near_half=46.0, far_half=300.0):
        ...
        self.puff_tint = np.array([0.92, 0.80, 0.64])        # dusty brown
    ...
    def set_textures(self, block_tex_list, arrow_layers, sky, tint_noise, puffs):
        ctx = self.ctx
        ...
        al = np.stack([np.ascontiguousarray(a, np.uint8) for a in arrow_layers])     # (V, 8, 32, 4) mip atlases
        self.t_arrows = ctx.texture_array((al.shape[2], al.shape[1], al.shape[0]), 4, al.tobytes())
    ...
    def render(self, cam, voxels=None, arrows=None, fx=None, near_center=(0.0, 0.0, 14.0), hurt=(0.0, 0)):
        """cam: dict(eye, target, fov, up(optional), roll(optional)).
        voxels: VOXEL_DTYPE instances; arrows: float32 (M, 10) pos3 quat4 scale1 variant1 fade1;
        fx: dict(puffs, streaks, lights); hurt: (overlay amount 0..1, number of leading voxel instances it tints)."""
        ctx = self.ctx
        iw, ih = self.iw, self.ih
        aspect = iw / ih
        near, far = 0.3, 1500.0
    ...
    def _particles(self, fx, view, proj, near, far, vp, cam):
        ...
        if all(x is None or not len(x) for x in (puffs, flashes, streaks)):
            return
        self.hdr_fbo.use()
        ctx.disable(moderngl.DEPTH_TEST | moderngl.CULL_FACE)
