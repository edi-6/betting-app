...
Brightness jumps on the frames where the zombie's red damage flash switches on or off are expected and are not
reported.
...
def main(video, cues_path):
    ...
    for i in range(1, len(cues)):
        if cues[i].get('hurt', 0.0) != cues[i - 1].get('hurt', 0.0):
            flash.update((i, i + 1))
    prev_key = None
